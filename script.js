...
window.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('loginForm').addEventListener('submit', function(e) {
        ...
        // Kullanıcı lisansı için - aynı anda sadece bir kişi kullanabilir
        const inUseKey = `license_inuse_${licenseKey}`;
        const isInUse = sessionStorage.getItem(inUseKey);
        
        if (isInUse && !activationTime) {
            // Başka biri kullanıyor ve bu ilk giriş
            message.textContent = currentLang === 'tr' ? 'Bu lisans şu anda başka biri tarafından kullanılıyor!' : 'This license is currently in use by someone else!';
            message.className = 'message error';
            return;
        }
        
        // Lisansı kullanımda olarak işaretle
        sessionStorage.setItem(inUseKey, 'true');
